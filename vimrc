...
" show tabline only if more than one
set showtabline=1
...
" useful for tpipeline plugin
" we can use this or put set -g default-terminal "xterm-256color"
" in tmux.conf
" to check for XT capability use tput XT in terminal
" let &t_fe = "\<Esc>[?1004h"
" let &t_fd = "\<Esc>[?1004l"
...
" vim must be compiled with clipboard option
" else install gvim which also has cli version of vim
vnoremap cp "+p
nnoremap cp "+p
