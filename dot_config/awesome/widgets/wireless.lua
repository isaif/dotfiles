...
-- local dir = os.getenv('HOME') .. '/.config/awesome/widgets/net_widgets/icons/'
-- local icons = {
...
local function draw_signal(level)
  if level == 0 then
    return icons.not_connected
  elseif level < 20 then
    return icons.level_one
  elseif level < 40 then
    return icons.level_two
  elseif level < 60 then
    return icons.level_three
  elseif level < 80 then
    return icons.level_four
  else
    return icons.level_five
  end
end
...
local function worker(args)
  ...
  -- Settings
  -- local ICON_DIR = awful.util.getdir('config')
  --   .. '/'
  --   .. module_path
  --   .. '/widgets/net_widgets/icons/'
  ...
  local widget = args.widget == nil and wibox.layout.fixed.horizontal()
    or args.widget == false and nil
    or args.widget
  ...
  local function net_update()
    ...
    if signal_level == nil then
      ...
      net_text:set_text(string.format(''))
