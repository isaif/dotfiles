...
--       params.with_action_column and create_textbox({ forced_width = 20 })
--         or nil,
...
local function worker(user_args)
  ...
  watch(CMD_slim, timeout, function(widget, stdout)
    ...
    local diff_idle = idle
      - tonumber(maincpu['idle_prev'] == nil and 0 or maincpu['idle_prev'])
    local diff_total = total
      - tonumber(maincpu['total_prev'] == nil and 0 or maincpu['total_prev'])
  ...
  popup_timer:connect_signal('timeout', function()
    awful.spawn.easy_async(CMD, function(stdout, _, _, _)
      ...
      for line in stdout:gmatch('[^\r\n]+') do
        ...
        local diff_idle = idle
          - tonumber(cpus[i]['idle_prev'] == nil and 0 or cpus[i]['idle_prev'])
        local diff_total = total
          - tonumber(
            cpus[i]['total_prev'] == nil and 0 or cpus[i]['total_prev']
          )
        ...
        --   local cmd = columns[5]
        --
        --   local kill_proccess_button = enable_kill_button
        --       and create_kill_process_button()
        --     or nil
        --
        ...
        --       local text = cmd
        --       if
        --         process_info_max_length > 0
        --         and text:len() > process_info_max_length
        --       then
